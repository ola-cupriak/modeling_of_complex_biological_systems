...
```{r}
pdf("Cupriak_problem1.pdf", width = 10, height = 10)
heatmap.2(edata,
          main = "Bottomly et al. Clustered rows and cols", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
```{r}
edata <- t(scale(t(edata), scale=FALSE, center=TRUE)) #scale=TRUE - scaling rows
svd.out <- svd(edata)
names(svd.out)
...
ggplot(PC) + geom_point(aes(x=V3, y=V4, col=as.factor(strain))) + ggtitle("Projection 3. and 4. principal components by genetic strains") + theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(loadings) + geom_point(aes(x=V1, y=V2, col=as.factor(strain))) + ggtitle("Projection of top 2 loadings of PCs by genetic strains") + theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
pdf("Cupriak_problem4.pdf",height=10,width=50)
loadings = data.table(svd.out$u,pData(bottomly.eset))

ggpubr::ggarrange(
          ggplot(loadings) + geom_violin(aes(x=as.factor(strain), y=V1),draw_quantiles = c(0.25, 0.5, 0.75)) + geom_jitter(aes(x=as.factor(strain), y=V1), alpha=0.2, color='orange'),
          ggplot(loadings) + geom_violin(aes(x=as.factor(strain), y=V2),draw_quantiles = c(0.25, 0.5, 0.75)) + geom_jitter(aes(x=as.factor(strain), y=V2), alpha=0.2, color='orange'),
          ggplot(loadings) + geom_violin(aes(x=as.factor(strain), y=V3),draw_quantiles = c(0.25, 0.5, 0.75)) + geom_jitter(aes(x=as.factor(strain), y=V3), alpha=0.2, color='orange'),
          ggplot(loadings) + geom_violin(aes(x=as.factor(strain), y=V4),draw_quantiles = c(0.25, 0.5, 0.75)) + geom_jitter(aes(x=as.factor(strain), y=V4), alpha=0.2, color='orange'),
          ggplot(loadings) + geom_violin(aes(x=as.factor(strain), y=V5),draw_quantiles = c(0.25, 0.5, 0.75)) + geom_jitter(aes(x=as.factor(strain), y=V5), alpha=0.2, color='orange'), 
          labels = c("1", "2", "3",'4','5'), ncol = 5, nrow = 1
        ) + ggtitle("Violin plots of the top 5 left singular vectors (loadings)") + theme(plot.title = element_text(hjust = 0.5))
```
...
# center the data - almost always
# scaling the data - when we know that variance in the data is the same
pc1 = prcomp(edata)
...
kdata <- kmeans(edata, centers=5)
tsne_out <- Rtsne(edata, pca=FALSE, perplexity=60)
tsne_out <- data.table(tsne_out$Y, cluster = as.factor(kdata$cluster))
ggplot(tsne_out) + geom_point(aes(x=V1, y=V2, col=cluster)) + ggtitle("t-SNE projection by k-means clusters") + theme(plot.title = element_text(hjust = 0.5))
```